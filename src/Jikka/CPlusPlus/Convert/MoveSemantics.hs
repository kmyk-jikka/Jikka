...
isMovableTo :: VarName -> VarName -> M.Map VarName VarName -> [[Statement]] -> Bool
isMovableTo x y0 env cont
...
runStatement stmt cont = case stmt of
  ...
  Declare t y init -> do
    ...
    env <- get
    case init of
      DeclareCopy (Var x) | (x `isMovableTo` y) env cont -> do
        modify' (M.insert y x)
        return []
